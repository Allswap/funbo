...
async function scanTriangularArb(
  ...
  const priority = ['0x0000000000000000000000000000000000001010','0x0d500b1d8e8ef31e21c99d1db9a6444d3adf1270','0x3c499c542cef5e3811e1192ce70d8cc03d5c3359','0x2791bca1f2de4661ed88a30c99a7a9449aa84174','0xc2132d05d31c914a87c6611c10748aeb04b58e8f','0x8f3cf7ad23cd3cadbd9735aff958023239c6a063','0x7ceb23fd6bc0add59e62ac25578270cff1b9f619','0x1bfd67037b42cf73acf2047067bd4f2c47d9bfd6','0x3a58a54c066fdc0f2d55fc9c89f0415c92ebf3c4','0xfa68fb4628dff1028cfec22b4162fccd0d45efb6','0x03b54a6e9a984069379fae1a4fc4dbae93b3bccd'];
  wellKnown.sort((a, b) => {
      ...
      for (let pIdx = 0; pIdx < shardPairs.length && workDone < maxPairsPerRun * maxRouterPairsPerPair; pIdx++) {
        ...
        for (let i = 0; i < routers.results.length && routerPairsDone < maxRouterPairsPerPair; i++) {
          for (let j = i + 1; j < routers.results.length && routerPairsDone < maxRouterPairsPerPair; j++) {
            ...
            const dB = await getTokenDecimals(_rpcUrl, pair.token_b, env);
            const scanAmountIn = ethers.parseUnits(String(scanTradeAmt), dA);
            // Quote A→B on both routers
